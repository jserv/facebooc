...
Connection *connectionCreate(sqlite3 *DB, int account1Id, int account2Id)
{
    ...
    if (sqlite3_bind_int(statement, 2, account1Id) != SQLITE_OK)
        goto fail;
    if (sqlite3_bind_int(statement, 3, account2Id) != SQLITE_OK)
        goto fail;

    if (sqlite3_step(statement) == SQLITE_DONE) {
        connection = connectionNew(sqlite3_last_insert_rowid(DB), t, account1Id,
                                   account2Id);
    }
                                      ...
                                      int account2Id)
{
    ...
    if (rc != SQLITE_OK)
        return NULL;
    if (sqlite3_bind_int(statement, 1, account1Id) != SQLITE_OK)
        goto fail;
    if (sqlite3_bind_int(statement, 2, account2Id) != SQLITE_OK)
        goto fail;
