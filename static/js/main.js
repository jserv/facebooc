...
(function($) {
  ...
  /* services */
  // service of retrieving page information
  window.main_app.service('serviceGetPageInfo', function($http){
    this.get_page_info = function(arg_path){
      return $http.get(arg_path);
    }
  });

  /* controllers */
  // headerController
  var headerController = function($rootScope, $scope, $http, $window, $location, serviceGetPageInfo, GLOBAL_VALUES){
    var ctrl = this;
    ...
    ctrl.init_header = function(){
      ...
      // get navbar info
      serviceGetPageInfo.get_page_info('/static/data/navbar_info.json').then(function(result){console.log(result);}, function(err){console.log(err);});
      // play beatbox midi
      ctrl.play_beatbox();
    }
  }
  ...
  var frontPageController = function($rootScope, $scope, $http, $window, $location, GLOBAL_VALUES){
    ...
    ctrl.map_handler = ctrl.map_handler || {
      ...
      init_map : function(arg_position){
        ...
        if( arg_position &&
            arg_position.coords &&
            (!isNaN(parseFloat(arg_position.coords.latitude)) && isFinite(arg_position.coords.latitude)) &&
            (!isNaN(parseFloat(arg_position.coords.longitude)) && isFinite(arg_position.coords.longitude)) ){
